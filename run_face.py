...
def check_in_mask(mask, box):
    center_x = (box[0] + box[2]) // 2
    print(center_x)
    if (center_x >= mask[0][0] and center_x <= mask[1][0]):
        return True
    return False
...
def main():
    ...
    mask_region = [[(540, 420), (667, 549)], [(798, 411), (954, 558)]]#, [(1419, 301), (1570, 453)], [(1575, 261), (1764, 438)]] #(mask_cam02_3)    
    ...
    while cap.isOpened and count <= 600*30:
        ...
        if ret:
            ...
            cv2.putText(org_frame, 'Current Emotion', (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                    ...
                    for i in range(len(face_list)):
                        ...
                        if (check_in_mask(mask_region[i], box)):
                            ...
                            if (no_face_count[i] >= time_to_live):
                                face_list[i].add_new_face(org_face, None, emo, count)
                            # Old face in region
                            else:
                                face_list[i].duration_container[-1].append(count)
